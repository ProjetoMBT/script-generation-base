...
public class script extends IteratingVUserScript {
	...
	public void run() throws Exception {
		...
		beginStep("[2] Moodle: Login to the site (/login/)", 0);
		{
			...
			web.textBox(7, "{{obj.moodleL.web_input_text_username}}").setText("lcosta");
			{
				think(0.337);
			}
			web.textBox(8, "{{obj.moodleL.web_input_text_username}}").pressTab();
			{
				think(0.586);
			}
			web.textBox(9, "{{obj.moodleL.web_input_password_password}}").setPassword(deobfuscate("+7Qav0cxWTQA0OpbEx8+dw=="));
			{
				think(1.689);
			}
